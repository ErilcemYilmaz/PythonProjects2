...
def fetch_data_from_api(name, legalSeatId):
    """
    Fetch data from the Zefix API for a given company name and legalSeatId.
...
def process_row(row):
    ...
    """
    name = row.get("name", "")
    legalSeatId = row.get("legalSeatId", "")
    data, success = fetch_data_from_api(name, legalSeatId)
    if success:
        return [
            dict(
                row,
